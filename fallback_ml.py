...
def train_fallback_model(df, target_col='dropout_label', exclude_cols=None):
    ...
    print("[INFO] Model trained successfully. Initializing SHAP explainer...")
    ...
    print("[SUCCESS] Fallback ML pipeline ready.")
...
def get_shap_factors(df):
    ...
    for i in range(len(shap_vals)):
        ...
        results.append({
            'top_shap_factors': json.dumps(factors_json),
            'shap_factor_1': push_factors[0][0] if len(push_factors) > 0 else 'baseline_risk',
            'shap_value_1': float(push_factors[0][1]) if len(push_factors) > 0 else 0.0,
            'shap_factor_2': push_factors[1][0] if len(push_factors) > 1 else 'baseline_risk',
            'shap_value_2': float(push_factors[1][1]) if len(push_factors) > 1 else 0.0,
            'shap_factor_3': push_factors[2][0] if len(push_factors) > 2 else 'baseline_risk',
            'shap_value_3': float(push_factors[2][1]) if len(push_factors) > 2 else 0.0,
        })
