...
async function loadStats() {
    try {
        ...
        const heroAtRisk = document.getElementById('hero-atrisk');
        if (heroTotal) animateCounter(heroTotal, data.total_students);
        if (heroAtRisk) animateCounter(heroAtRisk, data.at_risk_predicted);
        ...
        if (kpiTotal) animateCounter(kpiTotal, data.total_students);
        if (kpiDropouts) animateCounter(kpiDropouts, data.actual_dropouts);
        if (kpiAtrisk) animateCounter(kpiAtrisk, data.at_risk_predicted);
        if (kpiHigh) animateCounter(kpiHigh, data.intervention_tiers.high);

        const kpiRate = document.getElementById('kpi-rate');
        if (kpiRate) kpiRate.textContent = `${data.dropout_rate}% rate`;

        // Pill counts
        const total = data.total_students;
        const pillAll = document.getElementById('pill-count-all');
        const pillHigh = document.getElementById('pill-count-high');
        const pillMedium = document.getElementById('pill-count-medium');
        const pillLow = document.getElementById('pill-count-low');

        if (pillAll) pillAll.textContent = total;
        if (pillHigh) pillHigh.textContent = data.intervention_tiers.high;
        if (pillMedium) pillMedium.textContent = data.intervention_tiers.medium;
        if (pillLow) pillLow.textContent = data.intervention_tiers.low;
...
function updatePagination(data) {
    ...
    if (pageInfo) {
        pageInfo.textContent = `Page ${data.page} of ${data.total_pages} (${data.total.toLocaleString()} records)`;
    }
    if (prevPage) prevPage.disabled = data.page <= 1;
    if (nextPage) nextPage.disabled = data.page >= data.total_pages;
}
