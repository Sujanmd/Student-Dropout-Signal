...
def load_and_process_data():
    ...
    try:
        ...
        print("[SUCCESS] Live Databricks Hook Successful! Loaded {} rows.".format(len(df)))
        ...
        # Check local cache first to avoid slow model training and SHAP computations
        cache_path = os.path.join(os.path.dirname(__file__), 'students_processed_cache.pkl')
        if os.path.exists(cache_path):
            print("[INFO] Loading pre-processed student dataset from local cache...")
            try:
                cached_df = pd.read_pickle(cache_path)
                print(f"[SUCCESS] Local Cache Loaded Successfully! ({len(cached_df)} rows)")
                return cached_df
            except Exception as cache_err:
                print(f"[WARN] Failed to load cache ({cache_err}). Recalculating...")
        ...
        try:
            ...
            shap_res = fallback_ml.get_shap_factors(df)
            for col in shap_res.columns:
                df[col] = shap_res[col]
        ...
        df_processed = _add_phase2_features(df)
        try:
            df_processed.to_pickle(cache_path)
            print("[INFO] Pre-processed dataset cached locally to speed up future startups.")
        except Exception as cache_err:
            print(f"[WARN] Could not write cache: {cache_err}")
        return df_processed
...
def api_risk_distribution():
    ...
    return jsonify({
        ...
        'tier_distribution': {k.lower(): int(v) for k, v in tier_counts.items()},
    })
